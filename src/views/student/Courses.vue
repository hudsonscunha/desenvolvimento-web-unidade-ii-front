<template>
    <v-container>
        ...
        <h1 class="font-weight-light">Todos os cursos</h1>
        <v-row justify="center">
            <v-card class="ma-3" max-width="344" v-for="course in coursesList" :key="course.id_c">
                ...
                <v-card-actions class="justify-end">
                    <v-btn color="indigo lighten-1" dark @click="joinIn(course.id_c)">participar</v-btn>
                </v-card-actions>
            </v-card>
        </v-row>
    </v-container>
...
export default {
    ...
    methods: {
        async joinIn(id_c) {
            try {
                const res = await this.axios.post(`/student/course/${id_c}`, this.student)
