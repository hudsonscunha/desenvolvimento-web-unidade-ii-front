<template>
    <v-container class="text-center">
        ...
        <v-alert class="text-left" text v-model="alert.show" :type="alert.type" dismissible>{{ alert.message }}</v-alert>
                ...
                <v-simple-table fixed-header>
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">Descição</th>
                            <th class="text-center">Tarefa</th>
                            <th class="text-center">Entrega de tarefa</th>
                            <th class="text-center">Meu envio</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="assignment in assignmentsList" :key="assignment.id_a">
                            ...
                            <td><v-icon color="red" @click="open(assignment.a_file)">mdi-file-pdf-box</v-icon></td>
                            <td v-if="assignment.d_file == null"><v-icon color="info" @click="openForm(assignment.id_a)">mdi-cloud-upload</v-icon></td>
                            <td v-else><v-icon color="success">mdi-cloud-check</v-icon></td>
                            <td><a @click="open(assignment.d_file)">{{ assignment.d_filename }}</a></td>
                        </tr>
                    </tbody>
                </v-simple-table>
            </v-col>
        ...
        <v-dialog v-model="add" max-width="450">
            <v-card>
                <v-card-title>Entrega de tarefa</v-card-title>
                <v-card-text>
                    <v-form ref="addForm" class="ma-3" @submit.prevent="addDelivery">
                        <v-file-input
                        label="Arquivo"
                        v-model="deliveryToAdd.d_file"
                        :rules="[(v) => !!v || 'O arquivo é obrigatório']"></v-file-input>
                        <v-btn block class="success mt-3" type="submit">enviar</v-btn>
                    </v-form>
                </v-card-text>
            </v-card>
        </v-dialog>
...
export default {
        ...
        async addDelivery() {
            ...
            if (valid) {
                try {
                    ...
                    const index = this.assignmentsList.findIndex(a => a.id_a == this.deliveryToAdd.id_a)
    ...
    created: async function () {
        ...
        if (this.student == null) {
            ...
            try {
                const res = await this.axios.get(`/student/assignments/${this.$route.params.id_c}/${this.student.id}`)
